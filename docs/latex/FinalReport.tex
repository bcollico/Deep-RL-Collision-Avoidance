...
ADD SOME STUFF HERE. MOVE THE REWARD FUNCTION HERE. TALK ABOUT ROTATION INVARIANCE\\
...
Because the optimal policy should be invariant to any coordinate transformation (rotation and translation), there is some redundancy in the joint state sjn. In order to get rid off this ambiguity, an agent-centric frame is defined, with the origin at the agent’s position, and the x-axis pointing toward the agent’s goal, that is :

An illustration can be found here.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{docs/latex/figures/rotated_state.png}
    \caption{Illustration of the rotational invariance for the policy}
\end{figure}



The considered action space is continuous with a policy $\policy{}(\state{ij}{})=\ctrl{i} \, : \, \mathbb{R}^{15}\mapsto\mathbb{R}^2$. Reasonable bounds will be placed on the commanded control input. Single-integrator dynamic will be used as the baseline model: 
\begin{alignat}{3}
...
In order to evaluate our network, we compare our network with ORCA over 1,000 sample trajectories on the following metrics :
...
In order to encourage interaction in training, we have divided the environment in two, and put robot 0 on the left half plane, and robot 1 on the right half plane. For each robot, we created 1,000 randoms initial states on each side of the half plane. The gap between the two initial states distributions can be explained as we constrain the robot not to be colliding at the initial state. The goal states are in the other half plane for each robot, so that there are some possible collisions happening. Note that some trajectories might be collision-free so that robots just go straight towards their goal at their preferred speed. Each trajectory is 100 step-long, and with 2 robots having 1,000 initial states, we get at maximum 200,000 states, but since the algorithm stops when the goal is reached, we often only compute around 100,000 points.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\textwidth]{docs/latex/figures/train_states_distribution.png}
    \caption{Plot of the initial states distribution for 2 robots with 1,000 goals}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\textwidth]{docs/latex/figures/train_goals_distribution.png}
    \caption{Plot of the initial goals distribution for 2 robots with 1,000 goals}
\end{figure}
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
     ...
     \begin{subfigure}[b]{0.49\textwidth}
         ...
         \label{fig:heatmap_x}
     \end{subfigure}
     \caption{Example of Rotation Invariance of Supervised Learning Value Network Output.}
\end{figure*}
