...
The resulting policy with be evaluated in unstructured environments (no modelled obstacles) with varying degrees of change from the original application.\\
...
Another objective is to have the same extra time spent to reach goals for both agents. This extra time is the time added compared to going straight toward goal at the preferred speed for each robot. TALK ABOUT THE METRICS here.
