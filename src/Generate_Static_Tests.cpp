 ...
 * with n bots with pre-defined initial states and goal states
 */
...
void setupScenario(RVO::RVOSimulator *sim, int n_agents, std::vector<float> dim_world, float dt, 
					std::vector<float> radius, std::vector<float> vmax, std::vector<RVO::Vector2> pos,
...
int main()
{
	...
	std::vector<RVO::Vector2> goals;
	std::vector<RVO::Vector2> pos_init;
	...
	std::ofstream training_data_file("data/static_tests.csv");
    ...
    while (episode < N_episodes){
		...
		switch (episode){
			case 0:
				pos_init.push_back(RVO::Vector2(-3, 0));
				pos_init.push_back(RVO::Vector2( 3, 0));
				goals.push_back(RVO::Vector2( 3, 0));
				goals.push_back(RVO::Vector2(-3, 0));
				break;
			case 1:
				pos_init.push_back(RVO::Vector2(-3, 2)); 
				pos_init.push_back(RVO::Vector2( 3, 2));
				goals.push_back(RVO::Vector2( 3, -2)); 	 
				goals.push_back(RVO::Vector2(-3, -2));
				break;
		}
        ...
        setupScenario(sim, n_agents, dim_world, dt, radius, vmax, pos_init, goals);
        ...
        while ((!reachedGoal(sim, 0.1, goals) &&
        ...
        delete sim;
		radius.clear(); vmax.clear(); goals.clear(); pos_init.clear(); buffer.str(std::string());
    }
