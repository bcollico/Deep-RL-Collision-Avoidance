...
def evaluate_value_fcn_propagate(value_fnc, s_initial_1, s_initial_2, visualize, dt=DT, animate=False, savestr=None):
    ...
    if animate:
        plot_animation(get_goal(xs1),
                    ...
                    get_radius(xs1),
                    get_radius(xs2))
    if not cadrl_successful and not collision : 
    ...
    if visualize:
        plot_traj(xs1, xs2, dt=dt, savestr=savestr)
    return avg_value_diff, avg_vel_diff, avg_extra_time, cadrl_successful, collision

def evaluate(value_fnc, visualize, num_episodes, data=None, data_path=FOLDER+"/test_data.csv", animate=False, savestr=None):
    if data is None:
    ...
    for ep in ep_list[:num_episodes]:
        ...
        res = evaluate_value_fcn_propagate(value_fnc, s_initial_1, s_initial_2, visualize=visualize, animate=animate, savestr=savestr+f'_{ep}')
...
if __name__=='__main__':
    ...
    post_rl_model_path_reward_1 = FOLDER+"/post_RL_value_model_reward_1/"
    ...
    post_rl_fnc_reward_1 = tf.keras.models.load_model(post_rl_model_path_reward_1)
    post_rl_fnc = tf.keras.models.load_model(post_rl_model_path)

    # print("Initial value model evaluation")
    # evaluate(value_fnc=initial_value_fnc, visualize=True, num_episodes=2, data=data, data_path=None, animate=False, savestr='original')
    # print("Post RL value model evaluation")
    # evaluate(value_fnc=post_rl_fnc_reward_1, visualize=True, num_episodes=2,  data=data, data_path=None, animate=False, savestr='reward_1')
    # print("Post RL value model evaluation")
    evaluate(value_fnc=post_rl_fnc, visualize=True, num_episodes=2,  data=data, data_path=None, animate=False, savestr='reward_original')
    # 